module Decisions
  class BackDecisionTree < BaseDecisionTree
    ...
    from(DecisionTree::NSM_CLAIMED_COSTS)
      .when(-> { nsm || (nsm_supplemental && no_existing_ref?(multi_step_form_session.answers)) })
    ...
    from(DecisionTree::NSM_ALLOWED_COSTS)
      ...
      .when(lambda {
              nsm_supplemental &&
              to_be_paid?(multi_step_form_session.answers) &&
              no_existing_ref?(multi_step_form_session.answers)
            })
      .goto(edit: DecisionTree::NSM_CLAIM_DETAILS)
      ...
      .when(-> { nsm_supplemental })
      .goto(edit: DecisionTree::NSM_CLAIMED_COSTS)
      .when(-> { (nsm_appeal || nsm_amendment) && no_existing_ref?(multi_step_form_session.answers) })
      .goto(edit: DecisionTree::NSM_CLAIM_DETAILS)
