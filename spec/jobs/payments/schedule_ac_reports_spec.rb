...
RSpec.describe Payments::ScheduleAcReports do
  ...
  before do
    allow(Payments::AcReportMailer).to receive(:notify).with(any_args).and_return(dummy)
  ...
  describe '#perform' do
    it 'sends an email to finance and clears the tmp/uploaded/reports directory' do
      travel_to current_date do
        subject.perform
        expect(Payments::AcReportMailer).to have_received(:notify).with(start_date, end_date, 'finance@example.com')
        expect(Payments::AcReportMailer).to have_received(:notify).with(start_date, end_date, 'manager@example.com')
