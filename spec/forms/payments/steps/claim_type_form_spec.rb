...
RSpec.describe Payments::Steps::ClaimTypeForm, type: :model do
  subject(:form) { described_class.new(claim_type:, multi_step_form_session:) }
  ...
  describe '#save' do
    ...
    context 'when claim_type is valid' do
      let(:claim_type) { 'assigned_counsel' }

      context 'equals the value already in the session (unchanged)' do
        before do
          multi_step_form_session[:claim_type] = 'assigned_counsel'
      ...
      context 'and differs from the value in the session (changed)' do
        before do
          multi_step_form_session[:claim_type] = 'non_standard_mag'
