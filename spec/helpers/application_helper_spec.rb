...
RSpec.describe ApplicationHelper, type: :helper do
  ...
  describe '#current_namespace' do
    context 'when controller path starts with nsm' do
      before do
        allow(helper).to receive_message_chain(:controller, :controller_path).and_return('nsm/some_controller')
      end

      it 'returns nsm' do
        expect(helper.current_namespace).to eq('nsm')
      end
    end

    context 'when controller path starts with prior_authority' do
      before do
        allow(helper).to receive_message_chain(:controller, :controller_path).and_return('prior_authority/some_controller')
      end

      it 'returns prior_authority' do
        expect(helper.current_namespace).to eq('prior_authority')
      end
    end

    context 'when controller path does not match any known namespace' do
      before do
        allow(helper).to receive_message_chain(:controller, :controller_path).and_return('some_other_controller')
      end

      it 'returns default' do
        expect(helper.current_namespace).to eq('default')
      end
    end
  end

  describe '#feedback_url' do
    context 'when current namespace is nsm' do
      before do
        allow(helper).to receive(:current_namespace).and_return('nsm')
      end

      it 'returns the nsm feedback URL' do
        expect(helper.feedback_url).to eq('https://www.smartsurvey.co.uk/s/62ZC6B/')
      end
    end

    context 'when current namespace is prior_authority' do
      before do
        allow(helper).to receive(:current_namespace).and_return('prior_authority')
      end

      it 'returns the prior authority feedback URL' do
        expect(helper.feedback_url).to eq('https://www.smartsurvey.co.uk/s/UPNBDF/')
      end
    end

    context 'when current namespace is default' do
      before do
        allow(helper).to receive(:current_namespace).and_return('default')
      end

      it 'returns the default feedback URL' do
        expect(helper.feedback_url).to eq('_blank')
      end
    end
  end
end
